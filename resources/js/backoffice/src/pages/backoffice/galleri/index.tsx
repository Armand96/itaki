...
import { getGalleri, postGalleri } from '../../../helpers';
...
const Index = () => {
    ...
    const fetchData = async (page = 1) => {
        setLoading(true);
        const res: LinkType[] = await getGalleri(`?page=${page}`);
        setDataPaginate(res);
        setLoading(false);
    }
    ...
    const postData = async () => {
        ...
        await postGalleri(data, formData?.id).then(() => {
            setModal(false);
            Swal.fire('Success', formData.id ? 'Update Banner Berhasil' : 'Input Banner Berhasil', 'success');
        }).catch((err) => {
            setModal(false);
            console.log(err)
    ...
    const columns = [
        { name: 'Deskripsi', row: (cell: Banners) => <div>{cell.name}</div> },
        { name: 'Kategori', row: (cell: Banners) => <div>{cell.name}</div> },
        {
