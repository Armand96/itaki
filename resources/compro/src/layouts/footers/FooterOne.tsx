...
export default function FooterOne() {
    ...
    useEffect(() => {
        Promise.all([
            ...
            setListData({
                sosmed: res[0]?.map((item: any) => ({
                    name: item.name,
                    href: item.link,
                    svg: sosmed.find((s) => s.name === item.name)?.svg || '',
                })),
                webSettings: res[1]?.data,
            })
        })
    }, [])
        ...
        <>
            <footer className="luminix-footer-section1">
                <div className="container-header">
                                    ...
                                    <ul>
                                        <li>
                                            <span style={{ display: 'inline-flex', alignItems: 'flex-start', gap: 8 }}>
                                                <Image width={22} height={25} src={location_img} alt="Location icon" style={{ minWidth: 22 }} />
                                                <span>
                                                    {listData?.webSettings?.filter((x: any) => x.name === "alamat")[0]?.value}
                                                </span>
                                            </span>
                                        </li>
                                        <li>
                                        ...
                                        <li>
                                            <a href="tel:+088" style={{ display: 'inline-flex', alignItems: 'center', gap: 8 }}>
                                                <Image width={24} height={25} src={call_img} alt="here is theme image" />
                                                {listData?.webSettings?.filter((x: any) => x.name === "telp")[0]?.value}
                                            </a>
                                        </li>
                                    </ul>
                            ...
                            <div className="col-12 col-lg-5">
                                <div className="luminix-footer-menu2 ml-50 mb-0">
                                    ...
                                    <div className="luminix-subscription-field">
                                        <iframe src={listData?.webSettings?.filter((x: any) => x.name === "alamat_maps")[0]?.value} style={{ border: "0", width: "100%", height: "300px" }} loading="lazy" referrerPolicy="no-referrer-when-downgrade" ></iframe>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div className="luminix-social-wrap wrap2 mx-auto ">
                        <ul>
                            {listData?.sosmed?.map((link, index) => (
                                <li key={index}>
